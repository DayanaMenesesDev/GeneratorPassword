...
const meterPW = document.getElementById('meterPW');

const generatePassword = (e) => {
    e.preventDefault()
    const passwordLength = pwLength.value;
    container__pwLength.value = passwordLength;
    let password = '';
    ...
    meterPW.value = result.score; 
    meterPW.classList.remove('meterPWC0', 'meterPW1', 'meterPW2' , 'meterPW3' , 'meterPW4')
    if ( meterPW.value >= 0 && meterPW.value <=4 ) {
        meterPW.classList.add(`meterPW${meterPW.value}`)
    }
}
...
btnGeneratedPW.forEach(btn => {
    ...
    btn.addEventListener('click',(e) => { 
        ...
        if (e.target.id == 'regeneratePW') {
            ...
            formGeneratedPassword.insertBefore(div,displayPW);
            setTimeout(() => {
